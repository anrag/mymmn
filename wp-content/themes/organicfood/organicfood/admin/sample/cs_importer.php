...
if ( ! function_exists( 'installSample' ) ) {
	function installSample(){
		...
		if(($select_data=='all')||($select_data=='slider')){
			...
			update_option('cs_organicfood_dummy_slider',1);
		}
		if(($select_data=='all')||($select_data=='grid')){
			cs_import_grid($i);
			update_option('cs_organicfood_dummy_grid',1);
		}
		if(($select_data=='all')||($select_data=='widget')){
			cs_import_widget($i);
			update_option('cs_organicfood_dummy_widget',1);
		}
		if($select_data=='all'){
			$wp_import->import(get_template_directory() . '/admin/sample/'.$i.'/sample.xml');
			update_option('cs_organicfood_dummy',1);
		}
				...
				if ( !is_array( $val ) ) {
					// FUNCTIA DE SCHIMBAT URL SI UPLOAD POZA IN FOLDERUL WP-CONTENT
						...
						if ( isImage ( $val ) ) {
							...
							$i = 'organicfood';
							$image_path_on_upload = explode( 'http://vts.joomexp.com/'.$i.'/wp-content/uploads/',$val);
