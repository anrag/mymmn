...
global $wpdb;
if ( empty( $url ) )
//return new WP_Error( 'no_image_url', __( 'No image URL has been entered.','organicfood' ), $url );
...
if ( 'bmp' == $ext ) {
return new WP_Error( 'bmp_mime_type', __( 'Image is BMP. Please use either JPG or PNG.','organicfood' ), $url );
}
...
if ( !$size )
return new WP_Error( 'file_path_getimagesize_failed', __( 'Failed to get $file_path information using "@getimagesize".','organicfood'), $file_path );
...
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (GIF)','organicfood' ) );
}
elseif ( IMAGETYPE_PNG == $orig_type ) {
if ( !imagepng( $new_image, $dest_file_name ) )
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (PNG).','organicfood' ) );
}
else {
...
return new WP_Error( 'resize_path_invalid', __( 'Resize path invalid (JPG).','organicfood' ) );
}
...
if ( !$new_size )
return new WP_Error( 'resize_path_getimagesize_failed', __( 'Failed to get $dest_file_name (resized image) info via @getimagesize','organicfood' ), $dest_file_name );
